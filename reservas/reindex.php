...
<html>
    ...
    <body><?php
        ...
        function dibujar_celda($tabla, $dia, $h, $lunes, $usuario_id)
        {
        ...
        if (isset($_GET['pistas_id'])) {
            $pistas_id = trim($_GET['pistas_id']);
        ...
        if (isset($_GET['lunes'])) {
            $lunes = trim($_GET['lunes']);
            $_SESSION['lunes'] = $lunes;
        } elseif (isset($_SESSION['lunes'])) {
            $lunes = $_SESSION['lunes'];
        } else {
            $dow = (int) date("w");
            $dif = $dow == 0 ? 6 : $dow - 1;
        ...
        if (isset($_POST['id'])) {
            ...
            $res = pg_query_params("delete from reservas
                                     where id = $1", array($id));
        } elseif (isset($_POST['fecha'], $_POST['hora'])) {
        ...
        <form action="reindex.php" method="get"><?php
            $res = pg_query("select * from pistas order by id"); ?>
        ...
        <table style="margin:auto">
            <td>
                <form action="reindex.php" method="get">
